...
set -euxo pipefail
export PKLDIR="/home/devops/mlperf_dataset/maskrcnn/dataset_scripts"
...
readonly _cont_name="object_detection20.04"
...
# Setup container
nvidia-docker run -it --rm --init --detach \
...
log_event(key=constants.CACHE_CLEAR, value=True, stack_offset=0)"
        ...
        echo 'docker exec -it "${_config_env[@]}" "${_cont_name}" ${TORCH_RUN} --nproc_per_node=${DGXNGPU} ./run_and_time.sh'
        # Run experiment
